...
public class DNA {
    ...
    public void createDatabaseOfProfiles ( String filename ) {
        ...
        int p = StdIn.readInt(); // reads number of profiles in in the file 
        database = new Profile[p]; //creates database array p length 

        for (int i = 0; i < p; i++) { // for loop in order to create the profile elements with known information. 
            ...
            database[i] = profile; // populates the database array with new profiles 
        }
    }
    ...
    public void readSTRsOfInterest ( String filename ) {
        ...
        int s = StdIn.readInt(); //reads int for num of STRs and length of STRsOfInterest Array
        STRsOfInterest = new String[s]; // creates and initializes STRsOfInterest array

        for (int i = 0; i < s; i++) {
            STRsOfInterest[i] = StdIn.readString(); //populates the array with STROfInterest
        }
     ...
     */
    public STR findSTRInSequence ( String sequence, String STR ) { //test the for loop if it works or not. 
        ...
        for (int i = 0; i <= sequence.length() - STR.length(); i++) {
            if (STR.equals(sequence.substring(i, i + STR.length())) ) {
                ...
                if (increment > numOfRepeats) {
                    numOfRepeats = increment; 
                }
            }
            else {
            increment = 0;
        ...
        return new STR(STR, numOfRepeats);
}
}
    /**
    ...
    public void createProfileSTRs ( Profile profile, String[] allSTRs ) {
        //initialize two arrays, one for each sequences STRs; 
        ...
        for (int i = 0; i < allSTRs.length; i++) {
            ...
            STR potentialSTR1 = findSTRInSequence(sequence1, STR);

            String sequence2 = profile.getSequence2();
            STR potentialSTR2 = findSTRInSequence(sequence2, STR);

            S1_STRs[i] = potentialSTR1;
            S2_STRs[i] = potentialSTR2;
        }
    ...
    public boolean identicalSTRs ( STR[] s1, STR[] s2 ) {
        ...
        return match; // update the return value
    }
