...
public class RUHungry {
    ...
    public void menu(String inputFile) {
        ...
        int numOfCategories = StdIn.readInt(); // reads the first number 
        StdIn.readLine(); //next line 
        menuVar = new MenuNode[numOfCategories];
        categoryVar = new String[numOfCategories];

        for (int i = 0 ; i < numOfCategories; i++) {
            String categoryname = StdIn.readLine(); //reads the name of the category 
            categoryVar[i] = categoryname; 

            int numofDish = StdIn.readInt(); // num of disheszz
            StdIn.readLine(); // next line
            MenuNode ptr = null;
            ...
            for (int j = 0; j < numofDish; j++) {
                ...
                int[] ids = new int[numofingredientids];
                
                for (int k = 0; k < numofingredientids; k++) {
                    ids[k] = StdIn.readInt();
                }
                StdIn.readLine(); //next line 
    
                Dish dish = new Dish(categoryname, nameofdish, ids);
	...
	if (stockVar[newNode.getIngredient().getID() % stockVarSize] == null) {
        stockVar[newNode.getIngredient().getID() % stockVarSize] = newNode;
    }
    else {
        newNode.setNextStockNode(stockVar[newNode.getIngredient().getID() % stockVarSize]);
        stockVar[newNode.getIngredient().getID() % stockVarSize] = newNode;
    }
    }
    ...
    for (StockNode ptr = stockVar[ingredientID % stockVarSize]; ptr != null; ptr = ptr.getNextStockNode()) {
    ...
    StockNode stockNodeptr;

	if (ingredientName == null) {
        stockNodeptr = findStockNode(ingredientID);
    }
    else {
        stockNodeptr = findStockNode(ingredientName);
    }

    stockNodeptr.getIngredient().setStockLevel(stockNodeptr.getIngredient().getStockLevel() + stockAmountToAdd);
	...
	for (int i = 0; i < menuVar.length; i++) { //interates through the menuvar
        for (MenuNode ptr = menuVar[i]; ptr != null; ptr = ptr.getNextMenuNode()) { //iterates through the menuNodes 
            double cost = 0.0; //cost variable that resets every new menunode
            int[] ids = ptr.getDish().getStockID();//finds the array of ids with all of the necessary ingredients

            for (int j = 0; j < ids.length; j++) { //with every ingredient in the stuff
                cost = cost + findStockNode(ids[j]).getIngredient().getCost(); //it finds the cost of the ingredient from the stockVar and adds it to the cost 
            }

            double price = cost * 1.2; // cost * 1.2 = price
            ptr.getDish().setPriceOfDish(price); //set price on menunode 
            ptr.getDish().setProfit(price - cost); //set profit on menunode
        }
    }
    ...
    public void createStockHashTable (String inputFile){
        ...
        stockVarSize = StdIn.readInt(); //reads the first integer which corresponds to the length of the hashtable
        stockVar = new StockNode[stockVarSize]; //initializes the stock hashtable (like an array)

        StdIn.readLine(); //next line inshaAllah

       while (!StdIn.isEmpty()) {
            int ID = StdIn.readInt(); //reads integer for ID of ingredient 
            StdIn.readChar(); //skips the space between the integer and the upcoming name 
            String name = StdIn.readLine(); //reads name by reading the rest of the line and going to the next line
            double cost = StdIn.readDouble(); //reads cost 
            int stocklevel = StdIn.readInt(); //reads the stock level right next to the cost 
            StdIn.readLine(); //everything is read on this line. I want it to skip to the next line to set up the next loop.

            Ingredient item = new Ingredient(ID, name, stocklevel, cost); //creates the ingredient item  
            StockNode stockNode = new StockNode(item, null); //create stocknode for item

            addStockNode(stockNode); //adds the stock node to the stupid hashtable for the stupid stockVar 
    ...
    public boolean checkDishAvailability (String dishName, int numberOfDishes){
        ...
        for(int i = 0; i < ids.length; i++) {
            int remainingingredients = findStockNode(ids[i]).getIngredient().getStockLevel() + numberOfDishes;
        ...
        return isPossibletoMake; // update the return value
    }
    ...
    public void order (String dishName, int quantity){
        ...
        boolean canBePrepared = checkDishAvailability(dishName, quantity * -1);
        String type = "order";
        String item = dishName;
        

        if (canBePrepared == false) {
           
            double profit = 0.00;
            TransactionData data = new TransactionData(type, item, quantity, profit, canBePrepared);
            
            addTransactionNode(data); //add failed node

            MenuNode stationaryptr = findDish(dishName); //initialize a first ptr to point where the first failed dish is. 
            MenuNode movingptr; //intialize a second ptr to iterate through linked list 

            if (stationaryptr.getNextMenuNode() == null) {
                movingptr = menuVar[findCategoryIndex(findDish(dishName).getDish().getCategory())]; // if the first failed dish is at the end of the linked list, i put the movingptr at the head
            }
            else {
                movingptr = stationaryptr.getNextMenuNode(); //failed dish is not at the end
            }

            while (movingptr != stationaryptr) {
                item = movingptr.getDish().getName();
                boolean canMake = checkDishAvailability(item, quantity * -1);

                if (canMake) {
                    profit = quantity * movingptr.getDish().getProfit();
                    TransactionData data2 = new TransactionData(type, item, quantity, profit, true);
                    addTransactionNode(data2);
                    int[] ids = movingptr.getDish().getStockID();

                    for (int i = 0; i < ids.length; i++) {
                        updateStock(null, ids[i], quantity * -1);
                    }
                    ...
                    TransactionData data2 = new TransactionData(type, item, quantity, 0.00, false); 
                    addTransactionNode(data2);
                    
                    if (movingptr.getNextMenuNode() == null) {
                        movingptr = menuVar[findCategoryIndex(findDish(dishName).getDish().getCategory())];
                    }
                    else {
                        movingptr = movingptr.getNextMenuNode();
                    }
                }
            }
            ...
            int[] ids = findDish(dishName).getDish().getStockID();
            for (int i = 0; i < ids.length; i++) {
                updateStock(null, ids[i], -1 * quantity);
            }
        }
    ...
    public void donation (String ingredientName, int quantity){
        ...
        if ((profit() > 50)&& (findStockNode(ingredientName).getIngredient().getStockLevel() - quantity >= 0)){
                success = true;
            }
    
        TransactionData data = new TransactionData(type, ingredientName, quantity, 0.00, success);
        addTransactionNode(data);
    ...
    public void restock (String ingredientName, int quantity){
        ...
        if (profit() - (quantity * cost) < 0) {
            profit = 0.00;
        }
        else {
            updateStock(ingredientName, -1, quantity);
            success = true;
            profit = -1 * quantity * cost;
        }


	TransactionData data = new TransactionData(type, ingredientName, quantity, profit, success);
    addTransactionNode(data);
    }
